...
# Set local variables:
config_join_field = "VALUE" # the field that it will attempt to join from
...
config_input_gdb = os.path.join(os.getcwd(),"test_points_run.gdb")
config_filters = range(1981,2001)
config_series = [range(2020,2040),range(2050,2070),range(2080,2100)]
...
for series in config_series:
        ...
        config_filters = series
        for table in tables:
                print "processing %s" % table

                if len(config_filters) > 0:
                        filter_found = False
                        for filter_string in config_filters:
