...
config_mega_table = r"E:\SNMeadows\Amphibs\BCM\BCM_tables.gdb\BCM_tmax_megatable" # the location of the table that everything will be joined to
config_input_gdb = r"E:\SNMeadows\Amphibs\BCM\BCM_tables.gdb"

# Set environment settings
arcpy.env.workspace = config_input_gdb


# this script NEEDs to load the log script
# then it needs to log:
# skipped tables
# skipped rows from tables (log the join field id)
# other errors


# List tables in mdb
tables = arcpy.ListTables("tmax1999nov")
...
def copy_data(from_table,to_table,column_name,use_insert_cursor = False):
	...
	if use_insert_cursor is True:
		...
		for upd_row in output_data:
			try:
				# print "%s" % str(upd_row.getValue(config_join_field))
				in_row_value = fetch_row(str(upd_row.getValue(config_join_field)))
                # print "%s - %s" % (str(upd_row.getValue(config_join_field)),in_row_value)
			except:
				print "Couldn't get value from row"
				raise
		
			try:
				upd_row.setValue(column_name[1],in_row_value)
			except:
				print "Couldn't set row value"
				raise
