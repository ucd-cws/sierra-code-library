...
#       Cleaned up the "is_output_shapefile" code to be a function instead of in every function
#       Added major error handling to multi_to_single to handle an arcgis crash related to specific records
...
try:
    import arcpy
    import arcgisscripting # this is an arcgis 10 program, but we need to be able to catch arcgisscripting errors
except:
...
if desc.dataType == "ShapeFile": # if arcpy reports that it's a shapefile
	is_input_shapefile = True # set the flag
else:
	is_input_shapefile = False # otherwise, set the flag to 0
...
def _mkdir(newdir):
    ...
    if os.path.isdir(newdir):
        ...
        os.makedirs(newdir) # makes the directory and all directories required to make it.

def get_min_year(datasrc):
    '''opens the dataset up and determines what the minimum year is by iterating through the records and saving the lowest year. Returns the year'''
    ...
    minval = row.YEAR_ # set the first val
    cnt = 0
    try:
        for row in rows:
...
def union_single(datasrc, current_year):
    try:
        union_input = get_name(datasrc)
        sfyu = datasrc + "u"

        # NEW SET OF TEMPORARY FILES, ALL START WITH "sf".  THESE DEAL WITH THE ISSUE OF POLYGONS FROM A SINGLE YEAR OVERLAPPING
...
def calc_shape_area(calc_input):
    ...
    try:
        ...
        sys.exit(0)
# ----------------------------------------------------------------------------
#  Dissolve the single fire year file on the SHAPE_AREA field and retain the year and name.
#  The name retained is the last one read by the code as it processes overlapping polygons.
#  This was arbitrary.
# ----------------------------------------------------------------------------
def dissolve_single(dissolve_input, current_year):
    try:
        sfyd = get_name("sfy%sd" % current_year)

        arcpy.Dissolve_management(dissolve_input, sfyd, "FIRE_AREA", "FIRE_NAME LAST", "SINGLE_PART") #Replaced "FIRE_LABEL" with "FIRE_NAME" to match dataset (zs)
        return sfyd
    except:
...
# ----------------------------------------------------------------------------
def add_field(datasrc, fieldname, field_type):
    try:
        datasource = get_name(datasrc)
        
        if (debug): print "Adding Field: " + fieldname + " to " + datasource
        arcpy.AddField_management(datasource, fieldname, field_type)
...
def union_fires(current_year, previous_year, first_pass):
    try:
        if (first_pass):
            shp_union = get_name("sfy" + str(current_year) + "dd")
        else:
            shp_union = get_name("fy%su" % previous_year) + ";" + get_name("sfy%sdd" % current_year)
...
def multi_to_single(datasrc, current_year):
    try:
        final_layer = get_name("fire" + str(current_year))
        datasource = get_name(datasrc)
        ...
        try:
            '''the script kept failing here for 2006 and throwing feature ids that failed. After removing them, it kept
            failing on different features. This processing prints an error so that you can tell if so much has been deleted
            that the data is now crap, but hopefully gets around the moving goalpost problem.'''
            # parse the error to figure out which row was bad
            # delete the row from the feature, then try again
            
            import re
            # get the OID field from the dataset because we'll need to use that in the cursor retrieval - it's not always "OBJECTID" and this will tell us what it is
            t_desc = arcpy.Describe(datasrc)
            oid_field = t_desc.OIDFieldName
            del t_desc

            # get the bad row from the error message - the row ID always follows "OID"
            matches = re.search('OID (\d+)',str(e))
            if matches:
                bad_oid = matches.group(1)
            else:
                raise e
            print "Multipart to singlepart failed and reported that OID %s was bad - removing and retrying" % bad_oid

            # get just the bad row - then delete it
            t_curs = arcpy.UpdateCursor(datasrc,"\"%s\" = %s" % (oid_field,bad_oid))
            for row in t_curs:
                t_curs.deleteRow(row)

            # close the cursor
            del row, t_curs

            # rerun this function and return the value
            return multi_to_single(datasrc,current_year)
        except:
            raise
    except:
        raise
...
def get_name(name_str):
    '''gets rid of all of the is_output_shapefile logic that's everywhere and replaces it with a single function'''

    if is_output_shapefile:
        return "%s.shp" % name_str
    else:
        return name_str
...
# Fill fire name field
            fill_field(sfyd, nm_fld, "[LAST_FIRE_NAME]", "VB")
