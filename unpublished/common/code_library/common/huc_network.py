...
def make_upstream_matrix(hucs, output_csv, include_self_flag=False, zones_name="HUC_12", all_passed=False):
	"""
	...
	:param zones_name:
	:param all_passed: signifies whether or not all hucs of interest were passed in as the "hucs" parameter. When False, we find all hucs upstream of those to use for each side of the matrix. When True, we just use the passed hucs
	:return:
	"""

	log.write("Exporting matrix", True)
	...
	list_comp = list(set(list_comp))  # need to deduplicate the list so that we don't duplicate rows in the output matrix
	...
	for huc in list_comp:  #  list comprehension merges all of the hucs in the upstream lists for every huc in the initial list with the hucs themselves so that we get one master list of all hucs that need to be in the matrix
		...
		if huc not in watersheds:  # it may be (for the "M" watersheds) that they aren't in the main list
			continue
	...
	# run automated tests on the output matrix
	test_upstream_matrix(output_path)


def test_upstream_matrix(matrix_file):
	"""
		Provided a HUC_12 matrix, runs data tests on it and reports out errors
	:param matrix_file: a path to a matrix file on disk - NOT a file handle
	:return:
	"""

	with open(matrix_file, 'rb') as file_handle:
		matrix_handle = csv.DictReader(file_handle)
		fields_loaded = matrix_handle.fieldnames
		fields = set(fields_loaded) - set(["HUC_12"])  # strip out the huc_12 field - leave it as a set because we'll want it that way soon
		huc_12_column = []
		for row in matrix_handle:
			huc_12_column.append(row["HUC_12"])
		huc_12_set = set(huc_12_column)
		missing_rows = list(fields - huc_12_set)
		extra_rows = list(huc_12_set - fields)

		data_flag = False

		if len(missing_rows) != 0:
			log.warning("Missing {0:s} rows in output matrix: {1:s}".format(str(len(missing_rows)), str(missing_rows)))
			data_flag = True

		if len(extra_rows) != 0:
			log.warning("Output matrix has {0:s} extra rows: {1:s}".format(str(len(extra_rows)), str(extra_rows)))
			data_flag = True

		if len(huc_12_column) != len(huc_12_set):
			num_duplicates = len(huc_12_column) - len(huc_12_set)
			log.warning("Output matrix has {0:s} duplicated rows: {1:s}".format(str(num_duplicates), str(count_occurrences(huc_12_column))))
			data_flag = True

		if len(fields_loaded) != len(fields):
			num_duplicates = len(fields_loaded) - len(fields)
			log.warning("Output matrix has {0:s} duplicated columns: {1:s}".format(str(num_duplicates), str(count_occurrences(fields_loaded))))
			data_flag = True
